                ...
                <form action="{{route('inventories.store')}}" method="post">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="form-group col-md-8">
                            ...
                            <div class="input-group">
                                <select
                                    class="form-control form-control-sm fornecedors_id @error('fornecedors_id') is-invalid @enderror"
                                    name="fornecedors_id"
                                    aria-describedby="fornecedors_id_add"
                                    id="products_fornecedors_id">
                                    <option value="">-- select an option --</option>
                                    @foreach($fornecedors as $fornecedor)
                                        @if(isset($stock))
                                            <option
                                                {{$stock->fornecedors_id == $fornecedor->id? 'selected' : ''}} value="{{$fornecedor->id}}">{{$fornecedor->fornecedor}}</option>
                                        @else
                                            <option
                                                {{old('fornecedors_id') == $fornecedor->id? 'selected' : ''}} value="{{$fornecedor->id}}">{{$fornecedor->fornecedor}}</option>
                                        @endif
                                    @endforeach
                                </select>

                                <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal"
                                        data-bs-target="#fornecedor_modal">
                                    <i class="fa fa-plus-circle"></i>
                                </button>
